...
const MiniForm = ({ eventId }: Props) => {
  ...
  // 1. Check if user is already registered on mount
  useEffect(() => {
    const checkStatus = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/registrations/status/${eventId}`, {
          method: 'GET',
          headers: {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: Remove ': any' and check instanceof Error
      console.error(error);
  ...
  if (!isSignedIn) {
    return (
      <div className='w-full h-fit bg-white/5 backdrop-blur-md border border-white/10 px-6 py-8 flex flex-col gap-4 rounded-xl shadow-xl text-center'>
        ...
        <p className='text-gray-400 text-sm'>You must be logged in to register for this event.</p>
  ...
  if (isRegistered) {
    return (
      <div className='w-full h-fit bg-green-500/10 backdrop-blur-md border border-green-500/50 px-6 py-8 flex flex-col gap-5 rounded-xl shadow-xl text-center'>
        ...
        <p className='text-gray-300 text-sm'>You have successfully registered for this event. Check your email for details.</p>
  ...
  // State 4: Logged In & Ready to Register
  return (
    <form onSubmit={handleRegister} className='w-full h-fit bg-white/5 backdrop-blur-md border border-white/10 px-6 py-8 flex flex-col gap-5 rounded-xl shadow-xl'>
      <h2 className='text-white font-semibold text-xl border-b border-white/10 pb-2'>Book Your Spot</h2>
      
      {/* Pre-filled Read-only fields */}
      <div>
        <label className='text-gray-400 text-xs uppercase tracking-wider'>Registered As</label>
