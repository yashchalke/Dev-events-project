...
const ScanTicket = () => {
    ...
    // Critical Fix: Use Ref for immediate locking to prevent rapid-fire scans causing crashes
    ...
    const verifyTicket = useCallback(async (ticketId: string) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/api/registrations/verify', {
                method: 'POST',
                headers: {
            ...
            // Wait 3 seconds before resetting to allow next scan
            setTimeout(() => {
    ...
    const onScanSuccess = useCallback(async (decodedText: string) => {
        // Immediate check against Ref to prevent multiple triggers
        if (isScanningRef.current) return; 
        
        isScanningRef.current = true; // Lock immediately
        setIsProcessing(true);        // Update UI
        ...
        if (isSignedIn && !scannerRef.current) {
            ...
            setTimeout(() => {
                ...
                html5QrCode.start(
                    ...
                    config,
                    onScanSuccess,
                    // Error callback (optional, omitted to keep console clean)
                    undefined 
                ).catch((err: unknown) => {
            ...
            <GeneralLayout>
                ...
                <div className="h-screen flex items-center justify-center text-white px-4 text-center">
                    <div className="bg-white/10 p-8 rounded-xl border border-white/10 backdrop-blur-md">
                        ...
                        <p>You must be an organizer or staff member to access the scanner.</p>
                    </div>
                </div>
            </GeneralLayout>
        );
    ...
    return (
        <GeneralLayout>
            ...
            {/* CSS HACK: Forces video to cover container (Removes Black Bars) */}
            <style>{`
                #reader video {
            ...
            <div className="min-h-screen pt-28 pb-10 px-4 flex flex-col items-center">
                <div className="text-center mb-8 animate-in fade-in slide-in-from-top-4">
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Ticket Scanner</h1>
                    <p className="text-gray-400">Logged in as: <span className="text-bg-main">{user?.fullName}</span></p>
                </div>
                ...
                <div className="w-full max-w-md bg-black rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(147,51,234,0.3)] border-4 border-white/10 relative aspect-square">
                    ...
                    {cameraError ? (
                        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-gray-900 z-30">
                            ...
                            <p className="text-gray-500 text-sm">Note: Camera access requires HTTPS or Localhost.</p>
                        </div>
                    ) : (
                        // Camera View
                        ...
                        <div className="absolute inset-0 bg-black/85 z-20 flex flex-col items-center justify-center backdrop-blur-sm p-6 text-center animate-in fade-in duration-200">
                            {statusMessage.includes("✅") ? (
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-green-400 text-lg font-medium">{statusMessage.replace("✅", "").trim()}</p>
                                </div>
                            ) : statusMessage.includes("⚠️") ? (
                                <div className="flex flex-col items-center">
                                    <div className="text-5xl mb-4">⚠️</div>
                                    <h2 className="text-2xl font-bold text-white mb-2">Error</h2>
                                    <p className="text-yellow-400 text-lg">{statusMessage.replace("⚠️", "").trim()}</p>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center">
                                    ...
                                    <h2 className="text-2xl font-bold text-white mb-2">Declined</h2>
                                    <p className="text-red-400 text-lg font-medium">{statusMessage.replace("❌", "").trim()}</p>
                                </div>
                            )}
                            <p className="text-gray-500 text-sm mt-8 animate-pulse">Resetting in 3s...</p>
                        </div>
                    )}
                </div>
